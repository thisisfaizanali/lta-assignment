...
**Still unresolved:** card utilisation, emergency savings and bounce history.
...
the lack of recent hard enquiries. Every line is drawn from the same `why`
...
property worth three times his ask, so the naive move is to route him
straight to a cheap loan against that property. The app deliberately does not: because
...
also worth being blunt about a second point: his ₹15,00,000 ask clears
*neither* maximum, not the lender estimate (₹5.6–5.7L) and not the safe
amount (₹1.8–4.3L). The
...
outright whether Ravi is routed to a secured product, and this app's answer
is no. That is a deliberate call rather than an oversight, and it deserves
numbers rather than an assertion. Routed to loan against property instead, his real
...
left unanswered, and the app correctly does not invent one.
...
- **Age caps tenure, but none of the three trigger it.** Priya (29), Ravi (42)
  ...
  cap never binds for any of them. `affordability.test.ts` carries a dedicated
  older-borrower case instead, proving the rule is live rather than decorative,
  since no persona here could show it.
