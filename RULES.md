...
came apart before M1b. A sync test in M2 will assert the pairing mechanically;
...
|---|---|---|---|---|---|---|---|---|
| `personal` | Personal loan | No | **10.0% – 24.0%** | 60 m | 2% of sanction, or a flat ₹6,500 at some banks — modelled as a **range** (§4.4) | — | 20× net monthly income, ₹40L absolute | Tuned |
...
| `business` | Unsecured business loan | No | **14.0% – 26.0%** | 48 m | 2% + GST | — | ₹50L | Coarse |
| `gold` | Gold loan | Yes | **9.0% – 24.0%** (banks 9–12, NBFCs 12–24) | 12 m bullet / 36 m EMI | 1% | Tiered 85/80/75 (§1) | ₹1 Cr | Coarse |
| `home` | Home loan | Yes | **7.5% – 11.0%** | 300 m | 0.5%, capped ₹10,000 | 80%, 75% above ₹75L | — | Coarse |
...
| `MUDRA_FIRST_TIME_LIKELY_LIMIT` | The ₹20L Tarun Plus tier is not a first-loan ceiling — it generally requires a previously repaid Tarun loan. A genuine first-timer (Ravi: "never taken a formal loan") more realistically tops out near **₹10 lakh**. **Routing still uses the ₹20L limit** — gating eligibility on the ₹10L figure would push a first-timer's ₹15L ask straight into the secured branch and defeat the row above, which is the opposite of what this section exists to do. The ₹10L figure is surfaced only as a caveat in the routing explanation, telling the borrower to ask about both tiers. | Real review finding — see the M6 review report. Basis: **my judgement**; the ₹10L figure is an approximation of the base Tarun tier, not independently verified against a primary SIDBI source (same caveat as the ₹20L figure above) |
...
**A deliberate consequence, found in review:** for informal income this 50% discount CAN put the estimated lender-side maximum *below* what the borrower could actually safely carry — the one income type where the assignment's central "a lender offers more than is safe" framing runs the other way. Not tuned away: 50% is already generous (many formal lenders assess this income at zero), and the app doesn't hardcode which number is larger — the Statement always points at whichever of the two actually binds (see `StatementScreen.tsx`'s `bindingCap`).

Checked directly against Anita, the persona this was first raised against: it does **not** reproduce for her once her real household costs are properly stated (a separate review finding, §3's "the bad-month rule" note above) — her costs collapse safeMax so far that lenderMax stays the far larger number for her too. The inversion is still real and reachable — `affordability.test.ts` has a dedicated low-outgoing informal borrower proving it — it just isn't Anita's story. Reported here rather than silently claiming a persona result that the corrected numbers don't support.
...
| Rule | Value | Why | Source |
|---|---|---|---|
| **the bad-month rule** | When a range is given, use the **low end**, not the mean | Anita earns ₹26–30k. Budgeting to ₹28k means a ₹26k month is a missed EMI. This is the core of the borrower-side model. Not yet a named constant — it is a selection rule `income.ts` applies (M3), not a numeric threshold. | my judgement |
